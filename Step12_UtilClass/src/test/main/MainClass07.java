...
public class MainClass07 {
	public static void main(String[] args) {
		...
		Member mem3 = new Member(3, "원숭이", "상도동");
		// 3. 위에서 생성된 Member 객체의 참조값을 members List 객체에 모두 담아보세요.
		members.add(mem1);
		members.add(mem2);
		members.add(mem3);
		// 4. members ArrayList 객체에 담긴 내용을 이용해서 회원 목록을 아래와 같은 형식으로 반복문 돌면서 출력해보세요
		// 번호 : 1, 이름:  김구라, 주소 : 노량진
		// 번호 : 2, 이름: 해골, 주소 : 행신동
		
		for(int i=0; i<members.size(); i++) {
			Member mems = members.get(i);
			System.out.println("번호 : "+mems.num+", 이름 : "+mems.name+", 주소 : "+mems.addr);
		}
